...
{
    public class Item_Dialog_AddImage : Android.Support.V4.App.DialogFragment
    {
        ...
        public static Item_Dialog_AddImage NewInstance()
        {
            var frag = new Item_Dialog_AddImage { Arguments = new Bundle() };
            return frag;
        }
        ...
        private void LnChoosPicture_Click(object sender, EventArgs e)
        {
            try
            {
                AddItemActivity.createItem.CheckPermission();
                AddItemActivity.createItem.GalleryOpen();
                MainDialog.CloseDialog();
            }
            catch (Exception)
            {
                Log.Debug("stateStep", "Item_Dialog_AddItem" + "LnChoosPicture_Click");
            }
        }
        ...
        private void LnTakePicture_Click(object sender, EventArgs e)
        {
            ...
            {
                AddItemActivity.createItem.CheckPermission();
                AddItemActivity.createItem.CameraTakePicture();

                MainDialog.CloseDialog();
            }
            catch (Exception)
            {
                Log.Debug("stateStep", "Item_Dialog_AddItem" + "LnTakePicture_Click");
            }
        }
