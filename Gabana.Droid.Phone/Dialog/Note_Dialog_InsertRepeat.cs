...
{
    public class Note_Dialog_InsertRepeat : Android.Support.V4.App.DialogFragment
    {
        ...
        public static Note_Dialog_InsertRepeat NewInstance(string _name, string _detailinsert, string _event)
        {
            name = _name;
            Detail = _detailinsert;
            Event = _event;
            var frag = new Note_Dialog_InsertRepeat { Arguments = new Bundle() };
            return frag;
        }
        ...
        private async void Btn_save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var DetailNote = JsonConvert.DeserializeObject<Note>(Detail);
                if (DetailNote != null)
                {
                    if (Event == "insert")
                    {
        ...
        async void DetailInsert(Note note)
        {
            try
            {
                var result = await noteManage.InsertNote(note);
                if (!result)
                {
                ...
                {
                    JobQueue.Default.AddJobSendNote((int)note.MerchantID, (int)note.SysNoteID);
                }
                else
                {
                    note.FWaitSending = 2;
                ...
                long fogusCate = 0;
                if (note.SysNoteCategoryID == null)
                {
                    fogusCate = 0;
                }
                else
                {
                    fogusCate = (long)note.SysNoteCategoryID;
                }
                DataCashing.EditNoteItem = null;
                NoteActivity.SetFocusNew(fogusCate, note);
                NoteActivity.noteActivity.RefreshData();
                AddNoteActivity.addNote.Finish();
            }
            catch (Exception ex)
            {
                _ = TinyInsights.TrackErrorAsync(ex);
                _ = TinyInsights.TrackPageViewAsync("DetailInsert at note_dialog_insert");
            }
        }

        async void DetailUpdate(Note note)
        {
            try
            {
                var result = await noteManage.UpdateNote(note);
                if (!result)
                {
            ...
            {
                _ = TinyInsights.TrackErrorAsync(ex);
                _ = TinyInsights.TrackPageViewAsync("DetailUpdate at note_dialog_insert");
            }
        }
    }
}
