        ...
        class GabanaDispatchTask : AsyncTask<long, Java.Lang.Void, long>
        {
            ...
            protected override long RunInBackground(params long[] @params)
            {
                try
                {
                    ...
                    else if (paraMethodID == JobQueue.SendTransMethodID)
                    {
                        ...
                        {
                            var listjoninfo = allPendingJobs.Where(x => x.Extras?.GetString("systransid_value").ToLower() == paraSysTransNo).ToList();
                            if (listjoninfo.Count > 1)
                            {
                                return -1;
                            }
                        }
            ...
            private async void SentItemStart()
            {
                try
                {
                    ...
                    Item item = new Item();
                    item = await itemManager.GetItemforSyncAndroid(paraMerchantID, paraSysItemID);
                    if (item != null)
                    {
