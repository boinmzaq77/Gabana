...
{
    internal class Bill_Adapter_Item : RecyclerView.Adapter
    {
        public event EventHandler<int> ItemClick;
        private static RecyclerView rcvTopping;
        private TranWithDetailsLocal tranWithDetails;

        public Bill_Adapter_Item(TranWithDetailsLocal tran)
        {
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
        {
            try
            {
                ...
                rcvTopping.SetAdapter(cart_Adapter_Option);
                rcvTopping.HasFixedSize = true;
                rcvTopping.SetItemViewCacheSize(20);
        ...
        {
            View itemView = LayoutInflater.From(parent.Context).Inflate(Resource.Layout.bill_adapter_item, parent, false);
            ListViewReceiptHolder vh = new ListViewReceiptHolder(itemView, OnClick);

            rcvTopping = itemView.FindViewById<RecyclerView>(Resource.Id.rcvTopping);
            var layoutManager = new LinearLayoutManager(parent.Context, 1, false);
            rcvTopping.SetLayoutManager(layoutManager);
