...
{
    public class MyQRReceipt_Dialog_ChooseMedia : AndroidX.Fragment.App.DialogFragment
    {
        ...
        public static MyQRReceipt_Dialog_ChooseMedia NewInstance()
        {
            var frag = new MyQRReceipt_Dialog_ChooseMedia { Arguments = new Bundle() };
            return frag;
        }
        ...
        private void LnChoosPicture_Click(object sender, EventArgs e)
        {
            try
            {
                bool check = PaymentActivity.payment_main.CheckPermission();
                if (check)
                {
                    PaymentActivity.payment_main.GalleryOpen();
                }
                Dismiss();
            }
            catch (Exception ex)
        ...
        private void LnTakePicture_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (check)
                {
                    PaymentActivity.payment_main.CameraTakePicture();
                }
                Dismiss();
            }
            catch (Exception ex)
