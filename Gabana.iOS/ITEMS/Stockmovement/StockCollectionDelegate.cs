using Foundation;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using UIKit;

namespace Gabana.iOS
{
    public class StockCollectionDelegate : UICollectionViewDelegate
    {
        public override bool ShouldSelectItem(UICollectionView collectionView, NSIndexPath indexPath)
        {
            StockCollectionViewCell cell = collectionView.CellForItem(indexPath) as StockCollectionViewCell;
            if (cell == null)
            {
                //collectionView.ScrollToItem(NSIndexPath.FromRowSection(0, 0), UICollectionViewScrollPosition.CenteredVertically, false);
                return true;
            }

            OnItemSelected?.Invoke(indexPath);
            return true;
        }

        #region Events
        public delegate void DateSelectedDelegate(NSIndexPath indexPath);
        public event DateSelectedDelegate OnItemSelected;
        #endregion

    }
}