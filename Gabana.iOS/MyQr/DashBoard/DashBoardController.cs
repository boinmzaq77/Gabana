...
namespace Gabana.iOS
{
    public partial class DashBoardController : UIViewController
    {
        ...
        ChartView chartViewSalePeriod, chartViewSalePayment, chartView3;
        ...
        public override async void ViewDidLoad()
        {
            ...
            try
            {
                ...
                selectBranch.Clicked += (sender, e) => {
                    ...
                    {
                        DashBranchPage = new DashBoardBranchController();
                    }
        ...
        async void GetDataDashboard()
        {
            try
            {
                ...
                List<Entry> SalesByPeriods = new List<Entry>();
                foreach (var item in reportHourlies)
                {
                ...
                var chart = new LineChart() { Entries = SalesByPeriods, LabelTextSize = 18, BackgroundColor = SKColor.Parse("#ffffffff"), LineMode = LineMode.Straight };
                chart.MinValue = 0;
                ...
                color.Add("#37AA52");
                color.Add("#F8971D");
                List<Entry> SalesByPayments = new List<Entry>();
                int i = 0;
                foreach (var item in result.salesByPayments)
                {
                    Entry entry = new Entry((float)item.percentTotal)
                    {
                ...
                var chart3 = new BarChart() { Entries = BestEmp, LabelTextSize = 18 };
                chart3.MaxValue = (float)sumAllAmount;
                chart3.MinValue = 0;
                chartView3.Chart = chart3;

                //BestEmployeeView
                Utils.SetConstant(BestEmployeeView.Constraints, NSLayoutAttribute.Height, (result.bestEmployees.Count * 100)+80);
                BestEmployeeView.LayoutIfNeeded();

                Utils.SetConstant(chartView3.Constraints, NSLayoutAttribute.Height, (result.bestEmployees.Count * 80)+60);
                chartView3.LayoutIfNeeded();

            }
            catch (Exception ex)
            {
                await TinyInsights.TrackErrorAsync(ex);
                Utils.ShowAlert(this, "ไม่สำเร็จ !", "ไม่มีข้อมูล");
            }
        ...
        void initAttribute()
        {
            ...
            lbl_emptyDashboard.Hidden = true;
            lbl_emptyDashboard.Text = "คุณยังไม่มีข้อมูลการขาย";
        ...
        void setupAutoLayout()
        {
            ...
            BestEmployeeView.HeightAnchor.ConstraintEqualTo(120).Active = true;
            ...
            chartView3.TopAnchor.ConstraintEqualTo(lblBestEmployee.SafeAreaLayoutGuide.BottomAnchor,2).Active = true;
            chartView3.LeftAnchor.ConstraintEqualTo(chartView3.Superview.LeftAnchor, 15).Active = true;
            chartView3.RightAnchor.ConstraintEqualTo(chartView3.Superview.RightAnchor, -15).Active = true;
            chartView3.HeightAnchor.ConstraintEqualTo(80).Active = true;
            #endregion
        }
