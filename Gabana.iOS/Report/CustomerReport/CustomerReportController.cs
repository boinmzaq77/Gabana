...
{
    public partial class CustomerReportController : UIViewController
    {
        ...
        ReportSelectCustomerController CusPAge = null;
        CustomerReportDetailController CusDetailReport = null;
        ...
        public override async void ViewWillAppear(bool animated)
        {
            ...
            if (isModifyCustomer)
            {
                ...
                lsCustomer = await customer.GetAllCustomer();
                if (lsCustomer.Count == listChooseCustomer.Count)
                {
                    lbl_chooseCategory.Text = Utils.TextBundle("allcustomer", "All Customer");
                    return;
                }
                else if(listChooseCustomer.Count == 0)
                {
                ...
                foreach (var item in listChooseCustomer)
                {
                    ...
                    {
                        lbl_chooseCategory.Text += "," + item.CustomerName;
        ...
        private async void SetDataCustomer()
        {
           ...
           lsCustomer = await customer.GetAllCustomer();
            ...
            if (listChooseCustomer == null || listChooseCustomer.Count == 0)
            {
                listChooseCustomer = lsCustomer;
            }

            if (lsCustomer.Count == listChooseCustomer.Count)
            {
                lbl_chooseCategory.Text = Utils.TextBundle("allcustomer", "Items");
                return;
            }
            else if (listChooseCustomer.Count == 0)
            {
                lbl_chooseCategory.Text = Utils.TextBundle("none", "Items");
                return;
            }
            else
            {
        ...
        void initAttribute()
        {
            ...
            lbl_CateroryHead.Text = Utils.TextBundle("filterbycustomer", "Filter By Customer");
            ...
            lbl_Category.Text = Utils.TextBundle("customer", "Customer");
            ...
            btnSelectCategory.AddGestureRecognizer(tapGestureCustomer);
            #endregion
            ...
            btnViewReport.TouchUpInside += (sender, e) => {
                ...
                if(listChooseCustomer.Count > 0)
                {
                    ...
                    CusDetailReport = new CustomerReportDetailController(listChooseCustomer, txtStartDate.Text +"-"+ txtEndDate.Text, filterType, Start, End, lsCustomer.Count, filename);
                    this.NavigationController.PushViewController(CusDetailReport, false);
                }
                else
                {
                    Utils.ShowAlert(this, Utils.TextBundle("failed", "Failed !"), Utils.TextBundle("plschoosecus", "Items"));
                }
        ...
        public void Customer_select(UIGestureRecognizer sender)
        {
            ...
            CusPAge = new ReportSelectCustomerController();
            this.NavigationController.PushViewController(CusPAge, false);
        }
