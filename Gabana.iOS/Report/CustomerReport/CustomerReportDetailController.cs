...
namespace Gabana.iOS
{
    ...
    public partial class CustomerReportDetailController : UIViewController
    {
        ...
        CustomerManage setCate = new CustomerManage();
        List<Customer> ListCustomer = new List<Customer>();
        ReportFilterController filterPage = null;

        List<SalesByCustomerResponse> cusResponses;
        ...
        public CustomerReportDetailController(List<Customer> cater,string name, int filter,string start , string end,int cuscount, string filename)
        {
            this.ListCustomer = cater;
            this.nameCus = name;
        ...
        async void setupCollection()
        {
            try
            {
                ...
                //customerName.Add("ลูกค้าทั่วไป");
            if (ListCustomer.Count == CusCount)
            {
            ...
            else
            {
                foreach (var item in ListCustomer)
                {
                    ...
                    if (lblNameCate.Text != "")
                    {
                        lblNameCate.Text += "," + item.CustomerName;
                    }
                    else
                    {
                        lblNameCate.Text = item.CustomerName;
            ...
            Gabana3.JAM.Report.SalesByCustomerRequest cusRequest = new Gabana3.JAM.Report.SalesByCustomerRequest();
            cusRequest.sysBranchIDs = new List<int>(ReportController.listChooseBranch.Select(x => int.Parse(x.BranchID)).ToList()) ;
            cusRequest.startDate = this.startdate;
            cusRequest.endDate = this.enddate;
            cusRequest.customerName = customerName;

             cusResponses = new List<Gabana3.JAM.Report.SalesByCustomerResponse>();
            var result = await GabanaAPI.GetDataReportSummaryDailyCustomer(cusRequest);
                if (result == null || result.Count == 0)
                {
                    btnfilter.Enabled = false;
                    SearchbarView.Hidden = true;
                    CustomerCollectionView.Hidden = true;
                ...
                else
                {
                    ...
                    CustomerCollectionView.Hidden = false;
            ...
            ReportCustomerDataSource report_adapter = new ReportCustomerDataSource(cusResponses, ListCustomer);
            CustomerCollectionView.DataSource = report_adapter;
        ...
        void initAttribute()
        {
            ...
            {
                //filter
                filterPage = new ReportFilterController(2);
            ...
            CustomerCollectionView.RegisterClassForCell(cellType: typeof(CustomerReportDataViewCell), reuseIdentifier: "CustomerReportDataViewCell");
        ...
        public void PDF(UIGestureRecognizer sender)
        {
            ...
            if (UIDevice.CurrentDevice.CheckSystemVersion(10, 0))
            {
                ...
                var r = new UIGraphicsImageRenderer(CustomerCollectionView.Bounds.Size);


                var img = r.CreateImage((UIGraphicsImageRendererContext ctxt) =>
                {
                    CustomerCollectionView.Layer.RenderInContext(ctxt.CGContext);
                    //View.Capture(true);
                });
                var r2 = new UIGraphicsPdfRenderer(CustomerCollectionView.Bounds, UIGraphicsPdfRendererFormat.DefaultFormat);
        ...
        public void EMail(UIGestureRecognizer sender)
        {
            ...
            {
                UIGraphics.BeginImageContextWithOptions(CustomerCollectionView.Bounds.Size, CustomerCollectionView.Opaque, 0);
                CustomerCollectionView.Layer.RenderInContext(UIGraphics.GetCurrentContext());
        ...
        public async void Share(UIGestureRecognizer sender)
        {
            try
            {
                ...
                var file = "SaleReport_Customer_" + datenamefile;

                ReportManager.createDetail("CustomerReport", file, cusResponses, null, null, null, null, null, null, lblBranch.Text);
        ...
        async void SearchBytxt()
        {
            ...
            ((ReportCustomerDataSource)CustomerCollectionView.DataSource).ReloadData(Cus);
            CustomerCollectionView.ReloadData();
        }
        async Task<List<SalesByCustomerResponse>> GetFilterCustomerList()
        {
            try
            {
                if (!string.IsNullOrEmpty(txtSearch.Text))
                {
                    var itemlst = this.cusResponses.Where(m => m.customerName.Contains(txtSearch.Text)).ToList();
                    return itemlst;
                }
                else
                {
                    return this.cusResponses;
                }
            }
            catch (Exception ex)
            {
        ...
        void SetupAutoLayout()
        {
            ...
            CustomerCollectionView.TopAnchor.ConstraintEqualTo(SearchbarView.SafeAreaLayoutGuide.BottomAnchor, 5).Active = true;
            CustomerCollectionView.RightAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.RightAnchor).Active = true;
