...
namespace Gabana.iOS
{
    ...
    public partial class EmployeeReportDetailController : UIViewController
    {
        UICollectionView EmployeeCollectionView;
        ...
        UserAccountInfoManage setCate = new UserAccountInfoManage();
        List<ORM.MerchantDB.UserAccountInfo> ListEmployee = new List<ORM.MerchantDB.UserAccountInfo>();
        ReportFilterController filterPage = null;

        List<SalesBySellerResponse> EmpResponses;
        ...
        public EmployeeReportDetailController(List<ORM.MerchantDB.UserAccountInfo> cater,string name, int filter,string start , string end,int cuscount, string filename)
        {
            this.ListEmployee = cater;
            this.nameCus = name;
        ...
        async void setupCollection()
        {
            try
            {
                ...
                if(ListEmployee.Count == CusCount)
                {
                    lblNameCate.Text += Utils.TextBundle("allcustomer", "Items");
                    foreach (var item in ListEmployee)
                    {
                        customerName.Add(item.UserName);
                    }
                }
                else
                {
                    foreach (var item in ListEmployee)
                    {
                        ...
                        if (lblNameCate.Text != "")
                        {
                            lblNameCate.Text += "," + item.UserName;
                        }
                        else
                        {
                            lblNameCate.Text = item.UserName;
                ...
                Gabana3.JAM.Report.SalesBySellerRequest empRequest = new Gabana3.JAM.Report.SalesBySellerRequest();
                ...
                empRequest.sellerName = customerName;

                 EmpResponses = new List<Gabana3.JAM.Report.SalesBySellerResponse>();
                var result = await GabanaAPI.GetDataReportSummaryDailySeller(empRequest);
                if (result == null ||  result.Count == 0)
                {  
                    btnfilter.Enabled = false;
                    SearchbarView.Hidden = true;
                    EmployeeCollectionView.Hidden = true;
                ...
                else
                {
                    ...
                    EmployeeCollectionView.Hidden = false;
                ...
                {
                    case 1:
                        EmpResponses = result.OrderByDescending(x => x.sumTotalAmount).ToList();
                        break;
                    case 2:
                        EmpResponses = result.OrderBy(x => x.sumTotalAmount).ToList();
                        break;
                    case 3:
                        EmpResponses = result.OrderByDescending(x => x.sellerName).ToList();
                        break;
                    case 4:
                        EmpResponses = result.OrderBy(x => x.sellerName).ToList();
                        break;
                    default:
                        break;
                }

                ReportEmployeeDataSource report_adapter = new ReportEmployeeDataSource(EmpResponses, ListEmployee);
                EmployeeCollectionView.DataSource = report_adapter;
        ...
        void initAttribute()
        {
            ...
            {
                //filter
                filterPage = new ReportFilterController(3);
            ...
            EmployeeCollectionView = new UICollectionView(frame: View.Frame, layout: itemflowLayoutList);
            EmployeeCollectionView.BackgroundColor = UIColor.White;
            EmployeeCollectionView.ShowsVerticalScrollIndicator = false;
            EmployeeCollectionView.TranslatesAutoresizingMaskIntoConstraints = false;
            EmployeeCollectionView.RegisterClassForCell(cellType: typeof(EmployeeReportDataViewCell), reuseIdentifier: "EmployeeReportDataViewCell");
            View.AddSubview(EmployeeCollectionView);
            #endregion
        ...
        public void PDF(UIGestureRecognizer sender)
        {
            ...
            if (UIDevice.CurrentDevice.CheckSystemVersion(10, 0))
            {
                ...
                var r = new UIGraphicsImageRenderer(EmployeeCollectionView.Bounds.Size);


                var img = r.CreateImage((UIGraphicsImageRendererContext ctxt) =>
                {
                    EmployeeCollectionView.Layer.RenderInContext(ctxt.CGContext);
                    //View.Capture(true);
                });
                var r2 = new UIGraphicsPdfRenderer(EmployeeCollectionView.Bounds, UIGraphicsPdfRendererFormat.DefaultFormat);
        ...
        public void EMail(UIGestureRecognizer sender)
        {
            ...
            {
                UIGraphics.BeginImageContextWithOptions(EmployeeCollectionView.Bounds.Size, EmployeeCollectionView.Opaque, 0);
                EmployeeCollectionView.Layer.RenderInContext(UIGraphics.GetCurrentContext());
        ...
        public async void Share(UIGestureRecognizer sender)
        {
            try
            {
                ...
                var file = "SaleReport_Employee_" + datenamefile;

                ReportManager.createDetail("EmployeeReport", file, EmpResponses, null, null, null, null, null, null, lblBranch.Text);
        ...
        async void SearchBytxt()
        {
            ...
            ((ReportEmployeeDataSource)EmployeeCollectionView.DataSource).ReloadData(Cus);
            EmployeeCollectionView.ReloadData();
        }
        async Task<List<SalesBySellerResponse>> GetFilterCustomerList()
        {
            try
            {
                if (!string.IsNullOrEmpty(txtSearch.Text))
                {
                    var itemlst = this.EmpResponses.Where(m => m.sellerName.Contains(txtSearch.Text)).ToList();
                    return itemlst;
                }
                else
                {
                    return this.EmpResponses;
                }
            }
            catch (Exception ex)
            {
        ...
        void SetupAutoLayout()
        {
            ...
            EmployeeCollectionView.TopAnchor.ConstraintEqualTo(SearchbarView.SafeAreaLayoutGuide.BottomAnchor, 5).Active = true;
            EmployeeCollectionView.RightAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.RightAnchor).Active = true;
            EmployeeCollectionView.LeftAnchor.ConstraintEqualTo(View.SafeAreaLayoutGuide.LeftAnchor, 0).Active = true;
            EmployeeCollectionView.BottomAnchor.ConstraintEqualTo(BottomView.SafeAreaLayoutGuide.TopAnchor,-10).Active = true;
            #endregion
