...
{
    public partial class AddCashController : UIViewController
    {
        ...
        public AddCashController()
        {
        }
        public AddCashController(CashTemplate cashguide)
        {
            this.cashguide = cashguide;
        }
        ...
        public override void ViewWillAppear(bool animated)
        {
            ...
            {
                Utils.SetTitle(this.NavigationController, Utils.TextBundle("editcash", "Edit Cash"));
            }
        ...
        public override async void ViewDidLoad()
        {
            try
            {
                ...
                lblAmount.Text = Utils.TextBundle("amount", "Amount");
                dummyNumberView.AddSubview(lblAmount);


                lblAmount2 = new UILabel
                {
                    TextAlignment = UITextAlignment.Center,
                    TextColor = UIColor.FromRGB(162, 162, 162),
                    TranslatesAutoresizingMaskIntoConstraints = false
                };
                lblAmount2.Font = lblDummy.Font.WithSize(15);
                lblAmount2.Text =  Utils.TextCURRENCYSYMBOLS(CURRENCYSYMBOLS);
                dummyNumberView.AddSubview(lblAmount2);
                ...
                btnAddDummy.SetTitle(Utils.TextBundle("savecash", "Save Cash"), UIControlState.Normal);
                ...
                btnAddDummy.TouchUpInside += async (sender, e) => {
                    ...
                    {
                        btnAddDummy.Enabled = false;
                        if (Convert.ToDouble(lblDummy.Text)<=0)
                        {
                            Utils.ShowMessage(Utils.TextBundle("pleaseentercash", "Please specify the amount of cash"));
                ...
                else
                {
                    ...
                    lblDummy.Text = Utils.DisplayDecimal(cashguide.Amount);
                }
                DECIMALPOINTDISPLAY = DataCashingAll.setmerchantConfig.DECIMAL_POINT_DISPLAY;
        ...
        async void InsertCashGuide()
        {
            try
            {
                var lst = await CashTemplateManage.GetAllCashTemplate(DataCashingAll.MerchantId);
                if (lst == null)
                {
                    btnAddDummy.Enabled = true;
                    return;
                ...
                var lstcash = new List<ORM.Master.CashTemplate>();
                ...
                {
                    Utils.ShowMessage(Utils.TextBundle("notcompletedata", "Please fill in all information."));
                    btnAddDummy.Enabled = true;
                    return;
                }
        ...
        private void SetBtnSave()
        {
            ...
            if (decimal.Parse(strValue) > 0 && strValue != Vat )
            {
                btnAddDummy.Enabled = true;
